        ...
        <div class="principal-edit">
            @if (count($publicaciones) > 0)
                @foreach ($publicaciones as $publicacion)
                    <div class="publi">
                        <div class="votes">
                            <svg data-publicacion-id="{{ $publicacion->id }}" xmlns="http://www.w3.org/2000/svg" height="1em"
                                viewBox="0 0 512 512">
                                <path
                                    d="M256 48a208 208 0 1 1 0 416 208 208 0 1 1 0-416zm0 464A256 256 0 1 0 256 0a256 256 0 1 0 0 512zM135.1 217.4c-4.5 4.2-7.1 10.1-7.1 16.3c0 12.3 10 22.3 22.3 22.3H208v96c0 17.7 14.3 32 32 32h32c17.7 0 32-14.3 32-32V256h57.7c12.3 0 22.3-10 22.3-22.3c0-6.2-2.6-12.1-7.1-16.3L269.8 117.5c-3.8-3.5-8.7-5.5-13.8-5.5s-10.1 2-13.8 5.5L135.1 217.4z" />
                            </svg>
                            <p class="p-votes">{{ $publicacion->votes }}</p>
                            <svg data-publicacion-id="{{ $publicacion->id }}" xmlns="http://www.w3.org/2000/svg"
                        ...
                        <div class="post">
                            <div class="info-user">
                                <h3 style="margin-left: 10px">{{ $publicacion->title }}</h3>
                            ...
                            <div class="actions">
                                ...
                                <div class="item">
                                    ...
                                    <p>{{ $publicacion->comentarios->count() }} Comments</p>
                                </div>
                                ...
                                <div class="item">
                                    <form action="{{ route('publicaciones.delete', $publicacion->id) }}" method="POST">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit">
                ...
                @endforeach
            @else
                <h2 class="no-post">There are no posts to show</h2>
            @endif
        </div>
    </div>
    ...
    <script>
        document.querySelectorAll('.delete-btn').forEach(btn => {
            btn.addEventListener('click', function(event) {
                ...
                axios.delete(`/publicaciones/${Id}/delete`)
                    .then(response => {
                        window.location.href = '{{ route('publicacion.user') }}';
                    })
                    .catch(error => {
